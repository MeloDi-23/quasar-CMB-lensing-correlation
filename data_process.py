...
def jackknife_label(catalogue, Nside):
    """
    Use healpy to devide catalogue into jackknife subsamples.
    """
    c = coo.SkyCoord(ra=catalogue['ra']*u.degree, dec=catalogue['dec']*u.degree)

    l = c.galactic.l.to(u.rad).value
    b = c.galactic.b.to(u.rad).value
    return hp.ang2pix(Nside, theta=np.pi/2-b, phi=l)
