...
# quasar_cata = fits.getdata('/uufs/astro.utah.edu/common/home/u6060319/quasar-CMBlening/data/SDSS16Q/quasar_z_cut.fits')
...
pos_r = hp.ang2vec(theta=data[:,0], phi=data[:,1])
z_r = data[:,2]
Nr = len(data)
w_r = np.ones(Nr)
...
Ntotal = Nr + Nquas
pos_t = np.vstack((pos, pos_r))
z_t = np.hstack((z_l, z_r.flatten()))
w_t = np.hstack((w_l, w_r))
...
r_bins = np.geomspace(1, 30, Nbins+1)        # unit: cMpc/h
...
def progress_bar(receiver, rate=10):
    ...
    rate = max(1, int(rate))
    with tqdm.tqdm(total=Ntotal) as pbar:
        count = 0
        while True:
            if receiver.recv():
                count += 1
...
args = np.hstack((pos_t, w_t.reshape((-1, 1)), z_t.reshape(-1, 1)))
assert args.shape == (Ntotal, 5)
print('start calculation...')

progress = Process(target=progress_bar, args=(receiver,30))
progress.start()
...
assert values.shape==weight.shape==(Ntotal, Nbins)

print('finish calculating')
print('completeness: {}'.format(1-np.sum(np.isnan(values).flat)/(Ntotal*Nbins)))

np.save('result_sub.npy', (values, weight))

print('start stacking and jackknife...')
sigma = np.zeros(Nbins)
sigma_err = np.zeros(Nbins)
for i in range(Nbins):
    weight_l = weight[:Nquas,i]
    value_l = values[:Nquas,i]
    idx = np.logical_not(np.isnan(value_l))
    value_l = value_l[idx]
    weight_l = weight_l[idx]
    weight_l /= weight_l.mean()

    weight_r = weight[Nquas:,i]
    value_r = values[Nquas:,i]
    idx = np.logical_not(np.isnan(value_r))
    value_r = value_r[idx]
    weight_r = weight_r[idx]
    weight_r /= weight_r.mean()

    sigma[i] = np.sum(weight_l*value_l)/weight_l.sum() - np.sum(weight_r*value_r)/weight_r.sum()
