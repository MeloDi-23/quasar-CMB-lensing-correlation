...
alm = fits.open('/uufs/astro.utah.edu/common/home/u6060319/quasar-CMBlening/data/Planck/MV/dat_klm.fits')
for l in range(9):
    for m in range(l+1):
        index = np.where(alm[1].data['index']==l*l+l+m+1)[0]
        alm[1].data['real'][index] = 0
        alm[1].data['imag'][index] = 0
...
np.save(f'./calculation_data/result_r={sep_min}_{sep_max}_{Nbins}_{name}_l_cut.npy', (values, weight))
...
result.to_csv(f'./results/result_r={sep_min}_{sep_max}_{Nbins}_{name}_l_cut')
