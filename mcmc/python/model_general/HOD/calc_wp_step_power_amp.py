...
import numpy as np
def N_c(logM, par):
    return 10**par.logAmp*0.5*(1 + erf((logM - par.lgMmin)/par.sig_lgM))
...
def chi_2(signal, cov_inv, par, wp_table_auto, wp_table_cross, logM, nh):
    ...
    nq = N_g(logM, nh, par)                 # number density
    n = 1.4328e-5                           # magic number from the number density calculation.
    return np.einsum('i,ij,j', delta, cov_inv, delta) + ((n - nq)/(n/10))**2
